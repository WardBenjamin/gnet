...
int ecc_make_key_seed(uint8_t p_publicKey[ECC_BYTES + 1], uint8_t p_privateKey[ECC_BYTES]) {
	uint64_t l_private[NUM_ECC_DIGITS];
	EccPoint l_public;
	unsigned l_tries = 0;

	ecc_bytes2native(l_private, p_privateKey);
	//if (vli_isZero(l_private)) return 0;
	...
	EccPoint_mult(&l_public, &curve_G, l_private, NULL);
	//if (EccPoint_isZero(&l_public)) return 0;
